...
class Board(object):
    ...
    def current_dqn_config(self):
        ...
        availbale shape: width*width
        """

        square_state = np.zeros((self.width, self.height, 2))
        available = np.full((self.width, self.height), float("-inf"))
        row_idx = np.array(self.availables) // self.width
        col_idx = np.array(self.availables) % self.height
        available[row_idx, col_idx] = 0

        if self.states:
            moves, players = np.array(list(zip(*self.states.items())))
            move_curr = moves[players == self.current_player]
            move_oppo = moves[players != self.current_player]
            curr_idx = self.current_player - 1
            square_state[(move_curr // self.width),
                          move_curr % self.height, curr_idx] = curr_idx + 1
            square_state[(move_oppo // self.width),
                          move_oppo % self.height, 1-curr_idx] = 2-curr_idx

        return square_state, available
